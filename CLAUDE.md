...
- What is built: transport config/environments/errors/HTTP, FA(3) codegen + the `build` DSL, offline XSD validation for FA(3) *and* auth documents, the `AuthTokenRequest` document, and the XAdES-BES signer. Nothing can be *sent* yet — the four HTTP calls of §4.2 do not exist, and neither does crypto or any session code.
...
Last verified green on 3.2.11 (2026-08-22, after the XAdES signer): 379 examples, 0 failures, line 100 / branch 97.18 / method 100, RuboCop clean.
