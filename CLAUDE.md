...
- **Phase 2 order:**
  ...
  4. Online sessions, send/status/UPO/download. **In progress.** Done: `KsefNumber` (CRC-8 verified against the documented example), `Auth::AccessToken` (expiry from `validUntil`, *never* by decoding the JWT — §4.3 excludes the `jwt` dep), and `Sessions::Online` (open/send/close). Left: status polling, UPO retrieval, and the `Ksef::Client` facade.
     ...
     - Poll on `150` for invoices, but note **interactive sessions have no `150` — they use `170` for closed** (§12.1). A poller written against the batch table waits for a code that never arrives.
...
Last verified green on 3.2.11 (2026-08-23, after the crypto module, the KSeF-token flow and the doc-consistency pass): 585 examples, 0 failures, line 100 / branch 96.79 / method 100, RuboCop clean. The crypto primitives were measured on both 3.2.11 and 4.0.6 before any code was written — `OpenSSL::PKey::PKey#encrypt` with explicit OAEP options, the NIST and FIPS vectors, and the 190-byte OAEP limit all behave identically. One difference worth knowing: a wrong-padding decrypt raises `OpenSSL::PKey::PKeyError` on 4.0.6 but `OpenSSL::PKey::RSAError` on 3.2.11, so **rescue `OpenSSL::OpenSSLError`** in any spec asserting a crypto failure.
