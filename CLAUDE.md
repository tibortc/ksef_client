...
- **Phase 2 order:**
  ...
  4. Online sessions, send/status/UPO/download. **In progress.** Done: `KsefNumber` (CRC-8 verified against the documented example), `Auth::AccessToken` (expiry from `validUntil`, *never* by decoding the JWT — §4.3 excludes the `jwt` dep), `Sessions::Online` (open/send/close), and `Sessions::Status` (single-shot reads plus deadline-bounded waits, with `InvoiceCodes`/`SessionCodes` and the `SessionState`/`InvoiceState`/`UpoPage` value objects). Left: **UPO retrieval** and the **`Ksef::Client` facade**.
