...
- What is built: transport config/environments/errors/HTTP, FA(3) codegen + the `build` DSL, offline XSD validation for FA(3) *and* auth documents, and the `AuthTokenRequest` document. Nothing can be *sent* yet — no signer, no crypto, no session calls.
