...
- **CI (GitHub Actions):** `test.yml` — matrix `["3.2","3.3","3.4","4.0","head"]` (head: `continue-on-error`), RuboCop job, coverage artifact. It runs on push and pull request **and on a 01:30 UTC schedule**, because `Gemfile.lock` is gitignored so every leg resolves fresh — which makes this the only tier that sees a dependency break, and a tier that runs only on pushes sees nothing in a quiet week. Twice in one month an upstream release broke `main` with no commit here (`json` 3.0.0, then faraday 2.14.4); the second sat red for five days under five consecutive green nightlies, which could not see it because `nightly.yml` runs only `--tag integration`. The scheduled leg posts no Coveralls status: there is no diff for it to refer to. `nightly.yml` — live TEST-env integration on schedule, creds from **environment** secrets on `ksef-test` (`docs/REFERENCE.md` §6a.3 — a repository secret is readable by every workflow in the repo). `release.yml` — tag-triggered, **RubyGems Trusted Publishing (OIDC)** — no long-lived API keys anywhere; a second job then creates the **GitHub release** from the CHANGELOG section for that version (`tasks/release_notes.rb`), after the push and with its own `contents: write`, so the publishing job never holds write access to the repository.
...
- **A tag is not a version.** `github.ref_name` is `v0.1.0`; the CHANGELOG is keyed on `0.1.0`. The mapping lives in `ReleaseNotes.version_from_tag`, once, with a test — and a spec reads `release.yml` and asserts the job *calls* it. It did not until 2026-09-03, which would have failed `announce` on every release; and because `announce` runs `needs: publish`, the half that cannot be undone is the half that would have succeeded. **A workflow step is the least-tested code in this repository and the only code that runs when a mistake is permanent** — so where a step calls into `tasks/`, test the call and not only the function.
