...
- **Phase 2 order:**
  ...
  4. Online sessions, send/status/UPO/download. **In progress.** Done: `KsefNumber` (CRC-8 verified against the documented example), `Auth::AccessToken` (expiry from `validUntil`, *never* by decoding the JWT — §4.3 excludes the `jwt` dep), `Sessions::Online` (open/send/close), and `Sessions::Status` (single-shot reads plus deadline-bounded waits, with `InvoiceCodes`/`SessionCodes` and the `SessionState`/`InvoiceState`/`UpoPage` value objects), and `UPO::Client` + `UPO::Document` (both routes, integrity-checked, bytes kept verbatim — §12.3). Left: **UPO validation** (the §14.3 trap: `upo-v4-3.xsd` fixes `NazwaPodmiotuPrzyjmujacego`, so a strict validator rejects every UPO TEST issues — it must be a diagnostic, never a gate, and must accept all six pinned examples) and the **`Ksef::Client` facade**.
